...
for rows in df.to_dict(orient="records"):
  if (rows["insurer"] == "Intact" and rows["type"] == "Rental"):
    ...
    doc.render(rows)
    output_path = output_dir / f"{rows['insured_name']} - {docx_filename}"
  ...
  if (rows["insurer"] == "Gore Mutual" and rows["type"] == "Rental"):
    ...
    output_path = output_dir / f"{rows['insured_name']} - {pdf_filename1}"
    output_path.parent.mkdir(exist_ok=True)
  ...
  if (rows["insurer"] == "Optimum" and rows["type"] == "Rental"):
    ...
    output_path = output_dir / f"{rows['insured_name']} - {pdf_filename2}"
    output_path.parent.mkdir(exist_ok=True)
  ...
  if (rows["insurer"] == "Wawanesa" and rows["type"] == "Rental"):
    ...
    output_path = output_dir / f"{rows['insured_name']} - {pdf_filename3}"
    output_path.parent.mkdir(exist_ok=True)
    with open(output_path, "wb") as output_stream:
  ...
  if (rows["insurer"] == "Wawanesa" and rows["type"] == "Revenue"):
    ...
    output_path = output_dir / f"{rows['insured_name']} - {pdf_filename4}"
    output_path.parent.mkdir(exist_ok=True)
    with open(output_path, "wb") as output_stream:
